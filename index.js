...
const employeeCollection = client.db('cottageHomeCareServices').collection('employees');
...
async function run() {
    try {
        ...
        app.get('/allmessages/:service', async (req, res) => {
            ...
            const messageLength = await messageCollection.find(query).toArray();
            const messages = await cursor.skip(page * size).limit(size).toArray();
        ...
        app.get('/employees', async (req, res) => {
            const query = {};
            // console.log(user);
            const result = await employeeCollection.find(query).sort({ _id: 1 }).toArray();
            res.send(result);
